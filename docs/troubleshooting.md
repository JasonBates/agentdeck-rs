...
| Nothing opens remotely | Confirm AgentDeck listens on loopback and the trusted TLS proxy targets port 9798 | AgentDeck does not create or modify proxy configuration. |
...
| Copilot card has no derived context | Confirm Herdr reports `agent = copilot` with a safe `kind = id` session | AgentDeck best-effort reads only the matching local `session-state/<id>/events.jsonl`; it never reads the Copilot DB, invokes the CLI, or uses cloud access. Invalid, missing, or malformed data remains a generic Herdr card. Copilot generated headings are disabled. |
